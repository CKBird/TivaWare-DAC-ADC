...
void ConfigureSSI (void) { //Configure SSI for OLED
	...
	ROM_SSIConfigSetExpClk(SSI1_BASE, SysCtlClockGet(), SSI_FRF_MOTO_MODE_0, SSI_MODE_MASTER, 1000000, 8);
...
void ConfigureTimer0()
{
	//Timer Configure and enable
  	...
  	ROM_TimerLoadSet(TIMER0_BASE, TIMER_A, (SysTclClockGet()-1)/10000); //Should give 10KHz
...
int main (void) {
	...
	ROM_TimerEnable(TIMER0_BASE, TIMER_A);  	
 	...
 	while(1)
 	{
 		...
 		SSIDataPut(SSI1_BASE, sendA); //These 4 may not work...
 		while(SSIBusy(SSI1_BASE));

 		SSIDataPut(SSI1_BASE, average);
 		while(SSIBusy(SSI1_BASE));

 		SSIDataPut(SSI1_BASE, sendB);
 		while(SSIBusy(SSI1_BASE));

 		SSIDataPut(SSI1_BASE, onescomp);
 		while(SSIBusy(SSI1_BASE));
